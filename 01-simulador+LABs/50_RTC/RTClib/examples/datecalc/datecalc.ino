// Cálculos y Conversiones de Fecha simples.
// Adaptado por: Sergio Fco. Hernández Machuca
// sfhm_clases@hotmail.com

// Librerías que se emplearán en esta aplicación:
#include <Wire.h>
#include "RTClib.h"
...
void setup () {
  ...
  while (!Serial); // for Leonardo/Micro/Zero
    ...
    DateTime dt7 (dt6.unixtime() + 3600);       // Una hora después.
    showDate("dt7", dt7);

    DateTime dt75 = dt6 + TimeSpan(0, 1, 0, 0); // Una hora después con la adición del "TimeSpan".
    showDate("dt7.5", dt75);

    DateTime dt8 (dt6.unixtime() + 86400L);     // Un día después.
    showDate("dt8", dt8);

    DateTime dt85 = dt6 + TimeSpan(1, 0, 0, 0); // Un día después con la adición del "TimeSpan".
    showDate("dt8.5", dt85);
